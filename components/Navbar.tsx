  ...
  const fetchUserInfo = async () => {
    try {
      ...
      const { data } = await axios.get(
        `${process.env.NEXT_PUBLIC_CHAT_EXPRESS_SERVER}/api/user/getuser/${session?.user?.email}`,
        config
      );
    ...
    <div className="navbar bg-black text-white shadow-sm sticky top-0 z-50">
      ...
      <div className="navbar-center ">
        <input
          type="text"
          placeholder="Search"
          className="input input-bordered w-32 md:w-sm lg:w-96 text-black"
        />
      </div>
            ...
            <Drawer>
              <div className="flex justify-center items-center gap-1.5">
                <div className="message btn btn-sm rounded-full bg-black border-none text-white hidden md:flex">
                  <div onClick={handelNavigateMassage}>
                    <MessagesSquare />
                  </div>
                </div>
                ...
                <div className="dropdown dropdown-end">
                  ...
                  >
                    ...
                    <li onClick={handelNavigateMassage}>
                      <a>Message</a>
                    </li>
                    <li>
                      <a>Notification</a>
                ...
                <DrawerTrigger asChild>
                  <Button className="cursor-pointer md:hidden flex items-center bg-black border border-red-500 text-white md:px-5 py-2 rounded-lg font-medium transition duration-300">
                    <span className="hidden">Add Question</span>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 24 24"
